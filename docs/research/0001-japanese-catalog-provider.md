...
| --- | --- | --- | --- | --- |
| [TCGdex REST API](https://tcgdex.dev/rest) and [card database](https://github.com/tcgdex/cards-database) | Public HTTPS GET API. The database repository carries an [MIT license](https://github.com/tcgdex/cards-database/blob/master/LICENSE), but no reviewed API terms state a service-level rate limit, one-day response caching right, or required display attribution. Pokémon artwork remains third-party IP. | The schema and live endpoints provide separate `ja`, `ko`, `zh-cn`, and `zh-tw` set/card catalogs with stable IDs, images, and collector-local IDs. | TCGdex does not provide the required native-JPY YuYuTei quote contract. | **Accept for the Eastern metadata proof of concept.** Keep pricing separate and retain the legal/operational caveat for production use. |
