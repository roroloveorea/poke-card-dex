...
export default async function EasternSetPage({ params, searchParams }: { params: Promise<{ language: string; id: string }>; searchParams: Promise<{ page?: string | string[] }> }) {
  ...
  try {
    ...
    return <section className="page-shell"><Link className="text-link" href={`/sets/eastern/${slug}`}>← {catalogName} sets</Link><p className="eyebrow">{languageName(set.language)} set · {set.releaseDate}</p><h1>{set.name}</h1><p className="page-status">Page {cardPage.page} of {totalPages} · {cardPage.totalCount} card printings</p>{cardPage.items.length ? <><EasternCardGrid cards={cardPage.items} catalogSlug={slug} /><nav className="pagination" aria-label="Card pages">{page > 1 && <Link className="text-link" href={`/sets/eastern/${slug}/${id}?page=${page - 1}`}>← Previous</Link>}{page < totalPages && <Link className="button" href={`/sets/eastern/${slug}/${id}?page=${page + 1}`}>Next →</Link>}</nav></> : <p className="empty-state">No card printings are available for this set.</p>}</section>;
  } catch {
