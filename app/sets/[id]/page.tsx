...
export default async function SetPage({ params, searchParams }: {
  ...
  try {
    ...
    return <section className="page-shell"><Link className="text-link" href="/sets">← All sets</Link><p className="eyebrow">{languageName(set.language)} set · {set.releaseDate}</p><h1>{set.name}</h1><SetControls action={`/sets/${id}`} rarities={rarities} rarity={rarity} order={requestedOrder} />{order && cardPage.quotedCount === 0 && <p className="empty-state" role="status">The provider has no market prices for this set yet, so price ordering cannot change the results.</p>}<p className="page-status">Page {cardPage.page} of {totalPages} · {cardPage.totalCount} card printings{rarity ? ` · ${rarity}` : ""}</p>{cardPage.items.length ? <><CardGrid cards={cardPage.items} /><nav className="pagination" aria-label="Card pages">{cardPage.page > 1 && <Link className="text-link" href={pageHref(cardPage.page - 1)}>← Previous</Link>}<div className="page-numbers">{Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => pageNumber === cardPage.page ? <span className="current-page" aria-current="page" key={pageNumber}>{pageNumber}</span> : <Link key={pageNumber} href={pageHref(pageNumber)}>{pageNumber}</Link>)}</div>{cardPage.page < totalPages && <Link className="button" href={pageHref(cardPage.page + 1)}>Next →</Link>}</nav></> : <p className="empty-state">No card printings are available for these filters.</p>}</section>;
